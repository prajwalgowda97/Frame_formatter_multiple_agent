...
sub simulate_test 
{
  ...
  my ($test) = @_;
   {   
    ...
    {
      system("irun -access +rwc -f ./../uvme/compile.f -svseed $seed +UVM_TESTNAME=$test  +define+UVM_REPORT_DISABLE_FILE_LINE -coverage all -covdut frame_formatter_top  -covworkdir /cov_work -covoverwrite -covfile ./cov_files/cov_cmd.cf -uvmhome CDNS-1.1d"); 
    }
    else
    {  
      system("irun -access +rwc -f ./../uvme/compile.f -svseed $seed +UVM_TESTNAME=$test +define+UVM_REPORT_DISABLE_FILE_LINE -uvmhome CDNS-1.1d"); 
    }
